...
_MESSAGES: dict[str, dict[str, str]] = {
    "zh": {
        ...
        # 签到 / 积分（联网）
        "chk.title": "一键签到（领取各账号每日积分）",
        "chk.skipped": "跳过（{why}）",
        "chk.inactive_hint": "提示：签到活动当前未在上游开启，接口可用但领取会返回「活动未开启」。等活动开启后重跑本命令即可。",
        "cred.title": "账号积分与额度",
        "gw.title": "OpenAI 兼容网关",
        "gw.models_title": "可用模型（{n} 个）",
        "gw.base_url": "接口地址：{url}",
        "gw.models": "模型：{n} 个   默认：{default}",
        "gw.auth": "需要 API Key：{state}",
        "gw.usage_hint": "把任意 OpenAI 客户端的 base_url 指到上面这个地址即可（api_key 随便填）。",
        "gw.stop_hint": "按 Ctrl+C 停止。",
        "gw.stopped": "网关已停止。",
        "gw.port_busy": "端口 {port} 无法监听：{err}",
        "gw.default_mark": "* = 默认模型",
        "chk.confirm_title": "一键签到？",
        "chk.confirm_desc": "将依次为 {n} 个账号领取每日签到积分。\n串行执行（不并发），避免触发上游风控。",
        "chk.no_accounts": "没有存有登录态的账号，无法签到",
    ...
    "en": {
        ...
        # checkin / credits (network)
        "chk.title": "One-click check-in (claim daily credits)",
        "chk.skipped": "skipped ({why})",
        "chk.inactive_hint": "Note: the check-in activity is currently not open upstream; the API works but claiming returns \"activity not open\". Re-run once it opens.",
        "cred.title": "Account credits",
        "gw.title": "OpenAI-compatible gateway",
        "gw.models_title": "Available models ({n})",
        "gw.base_url": "Endpoint: {url}",
        "gw.models": "Models: {n}   default: {default}",
        "gw.auth": "API Key required: {state}",
        "gw.usage_hint": "Point any OpenAI client's base_url here (api_key can be any value).",
        "gw.stop_hint": "Press Ctrl+C to stop.",
        "gw.stopped": "Gateway stopped.",
        "gw.port_busy": "Cannot bind port {port}: {err}",
        "gw.default_mark": "* = default model",
        "chk.confirm_title": "Check in now?",
        "chk.confirm_desc": "Will claim daily credits for {n} account(s), one at a time\n(serial, not concurrent) to avoid upstream rate limiting.",
        "chk.no_accounts": "No account has a saved login session",
        "cli.header": "  {idx:<4} {name:<24} {uid:<38} {sessions:>9} {memory:>10}",
    },
}
